...
<%
  ...
  // Get updated appointment information from form submission
  ...
  String query = "UPDATE appointments SET department=?, phone_number=?, name=?, email=?, appointment_date=?, appointment_time=? WHERE id=?";
  ...
  if (rowsUpdated > 0) {
    response.sendRedirect("show_appointments.jsp?message=success");
  } else {
    response.sendRedirect("show_appointments.jsp?message=error");
  }
%>
